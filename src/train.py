""" 
Este módulo é responsável por treinar o modelo de previsão de RUL utilizando
o dataset da NASA C-Maps. Ele segue as seguintes etapas:
...
def run_training_pipeline():
    ...
    if not data_path.exists():
        raise FileNotFoundError(f"Arquivo não encontrado: {data_path}."
                                f"Execute o pipeline de dados primeiro."
        )
    print("Carregando dataset processado...")
    df = pd.read_csv(data_path)

    # 2. Separação de Features (X) e Target (y)
    # Exclusão das colunas de identificação e permanência dos sensores e settings para prever o RUL
    ...
    with mlflow.start_run() as run:
        ...
        # Instancia do modelo com os hiperparâmetros do config.py
